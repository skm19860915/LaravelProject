...
class FirmChatController extends Controller
{
    ...
    {
        $data = Auth::User();
        $users = User::where('firm_id', '=',$data->firm_id)->where('id', '!=', $data->id)->get();
        return view('firmadmin.firmclient.chat.index', compact('users'));
    }
    ...
    public function chat(Request $request, $ids)
    {
        ...
        return view('firmadmin.firmclient.chat.chat', compact('users', 'otherUser'));
    }
}
