...
<div id="UploadClinetDocument" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{url('admin/allcases/setAdminCaseDocument')}}" method="post" enctype="multipart/form-data">
          <div class="row">
...
$(window).on('load', function() {
  ...
  function getdocdata(fid) {
    if(fid) {
      index_url = "{{url('admin/allcases/getFamilyDataDocument')}}/{{$case->id}}/"+fid;
    }
    else {
      index_url = "{{url('admin/allcases/getCaseDataDocument')}}/{{$case->id}}";
    }
    $('#table').DataTable({
      processing: true,
      serverSide: true,
      ...
      columns: [
        ...
        { data: null,
          render: function(data){
            ...
            view_button = '<a href="#" data-id="'+data.id+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn viewdocbtn" title="View" data-toggle="tooltip"><img src="{{url('assets/images/icon')}}/Group 557.svg" /></a>';

            view_button += ' <a href="{{url('admin/allcases/editrequestdocuments')}}/{{$case->id}}/'+data.id+'" class="action_btn" title="Edit Request" data-toggle="tooltip"><img src="{{ url('/') }}/assets/images/icon/pencil(1)@2x.png"></a>';

            view_button += ' <a href="{{url('admin/allcases/uploaddocuments')}}/{{$case->id}}/'+data.id+'" class="action_btn" data-toggle="tooltip" title="Upload Document"><i class="fas fa-upload"></i></i></a>';
...
$(document).on('click', '.convert_client_act', function(e){
  ...
  if(file_type != '') {
    if(family_id != '') {
      $.ajax({
        url: "{{url('admin/allcases/setAdminCaseDataDocument')}}",
...
$(document).ready(function(){
  ...
  $('.request_doc').on('click', function(e){
      ...
      $.ajax({
        type:"post",
        url:"{{ url('admin/allcases/rquestblueprintdocuments1') }}",
  ...
  $('.completeform').on('click', function(){
      ...
      $.ajax({
       type:"get",
       url:"{{ url('admin/allcases/completeDocument4') }}/"+v,
