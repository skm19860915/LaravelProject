...
<div class="modalformpart" id="modal-form-part" style="display: none;">
  ...
  <div class="row">  
    ...
    <div class="col-md-8">
      ...
      <select class="selectpicker1" name="who_consult_with" required="required" style="height: 150px;">
        <?php if(!empty($firmadmins)) { ?>
        @foreach ($firmadmins as $l)
          <option value="{{$l['id']}}" data-firm_id="{{$l['firm_id']}}">{{$l['name']}}</option>
        @endforeach
        <?php } ?>
      </select>
    </div>
  ...
  <div class="row firmcase" style="display: none;">  
    ...
    <div class="col-md-8">
      ...
      <select class="selectpicker1" name="firmcase" style="height: 150px;">
        ...
        @foreach ($firmcases as $l)
          <option value="{{$l['id']}}" data-firm_id="{{$l['firm_id']}}">{{$l['case_type']}}</option>
        @endforeach
        <?php } ?>
      </select>
    </div>
  ...
  <div class="row firmclient" style="display: none;">  
    ...
    <div class="col-md-8">
      ...
      <select class="selectpicker1" name="firmclient" style="height: 150px;">
        <option value="">select</option>
        <?php if(!empty($firmclients)) { ?>
        @foreach ($firmclients as $l)
          <option value="{{$l['id']}}" data-firm_id="{{$l['firm_id']}}">{{$l['name']}}</option>
        @endforeach
        <?php } ?>
      </select>
...
<script>
  $(document).ready(function(){
    ...
    $('.saveclientinfo_form').on('click', function(e){
      ...
      $.ajax({
        ...
        {       
          res = JSON.parse(res);
          if(res.status) {
            window.location.href = "{{ url('admin/usercalendar') }}";
          }
          else {
            alert(res.msg);
          }
    ...
    $('select[name="who_consult_with"]').on('change', function(){
      var v = $(this).val();
      var fid = $('select[name="who_consult_with"] option[value="'+v+'"]').data('firm_id');
      $('select[name="firmcase"] option').hide();
      $('select[name="firmclient"] option').hide();
      $('select[name="firmcase"]').val('');
      $('select[name="firmclient"]').val('');
      $('select[name="firmcase"] option[data-firm_id="'+fid+'"]').show();
