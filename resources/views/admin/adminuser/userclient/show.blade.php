...
<section class="section">
        ...
        <div class="col-md-12">
          ...
          <div class="card-header">
            ...
            <div class="card-header-action">
              <a href="{{url('admin/userclient/forms')}}/{{$id}}" class="btn btn-primary">Forms <i class="fas fa-eye"></i>
              </a>
              &nbsp;&nbsp;&nbsp;&nbsp;
              <a href="{{url('admin/document_request')}}/{{$id}}" class="btn btn-primary">Request Document <i class="fas fa-eye"></i>
              </a>
            </div>
          </div>
          <!-- <button class="btn btn-primary trigger--fire-modal-2" id="fire-modal-2" style="width:200px;">Add Client Information form</button> -->
  ...
  <div class="" id="modal-form-part">
    <input type="hidden" name="client_id" value="{{$client->id}}" class="form-control">
      <select class="form-control" name="file_type">
        <option value="">select</option>
        <option value="family_information">Family Information</option>
      </select>
      @csrf
...
<script type="text/javascript">
 $(document).ready(function(){
  $("#fire-modal-2").fireModal({title: 'Client Information Form', body: $("#modal-form-part"), center: true});
});
$(document).on('click', '.saveclientinfo_form', function(){
  var file_type = $('select[name="file_type"]').val();
  var client_id = $('input[name="client_id"]').val();
  var csrf1 = $('input[name="_token"]').val();
  if(file_type != '') {
    $.ajax({
      url: "{{route('admin.userclient.setDataClient')}}",
      data: {file_type: file_type, client_id: client_id, _token: csrf1},
      dataType: 'json',
      type: 'post',
      async: false,
      success: function (data) {
        console.log('success===========',data);
        alert('Client Information form submited');
    ...
    alert('Please select Client Information Form');
  }
 });
</script>
@endpush 