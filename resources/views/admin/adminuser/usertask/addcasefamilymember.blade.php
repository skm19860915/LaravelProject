...
<!--new-header Close-->
  <div class="section-body">
      <div class="row">
        <div class="col-md-12">
          <div class="card">
        <form action="{{ url('admin/usertask/createcasefamily') }}" method="post" class="needs-validation" enctype="multipart/form-data" novalidate="">
          ...
          <div class="card-body">
            ...
            <div class="form-group row mb-4">
              <label class="col-form-label text-md-right col-12 col-md-2 col-lg-2"> Member Of
              </label> 
              <div class="col-sm-12 col-md-7">
                <select class="selectpicker" name="member_of" data-live-search="true">
            ...
            <div class="form-group row mb-4">
              <label class="col-form-label text-md-right col-12 col-md-2 col-lg-2"> Relationship
              </label> 
              <div class="col-sm-12 col-md-7">
                <select class="selectpicker" name="member_relationship" data-live-search="true">
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <a href="#" class="btn btn-primary addexistingmember">Save</a> <strong style="margin-right: 10px;">Or</strong> <a href="#" class="btn btn-primary addnewmemberbtn">Add new member</a>
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                <div class="form-client-edit-input">
                  <div class="row">
                    <label class="col-md-4 col-sm-4 col-xs-12">  Member Of
                    </label> 
                    <div class="col-md-8 col-sm-8 col-xs-12">
                      <select class="selectpicker" name="beneficiary" data-live-search="true">
                        <?php 
                if($firm->account_type == 'CMS') { ?>
                        <option value="{{$client->user_id}}">{{$client->first_name}} {{$client->middle_name}} {{$client->last_name}}</option>
                        <?php }
...
$(document).ready(function(){
  ...
  $('.addexistingmember').on('click', function(){
    ...
    var member_of = $('select[name="member_of"]').val();
    var member_relationship = $('select[name="member_relationship"]').val();
    if(family_id == '') {
      alert('Please select member');
    }
    else if(member_of == '') {
      alert('Please member of');
    }
    else if(member_relationship == '') {
      alert('Please member relationship');
    }
    else {
      var _token = $('input[name="_token"]').val();
      $.ajax({
        type:"post",
        url:"{{ url('admin/usertask/addfamilymemberincase') }}",
        data: { 
          family_id: family_id, 
          member_of: member_of,
        ...
        success:function(res)
        {       
          window.location.href = "{{ url('admin/usertask/casefamily') }}/{{$id}}";
        }
      });
  ...
  $('select[name="family_id[]"]').on('change', function(){
    var v = $(this).val();
    $('select[name="member_of"]').val('');
    $('select[name="member_of"] option').show();
    $('select[name="member_of"] option[value="'+v+'"]').hide();
    $('select[name="member_of"]').selectpicker('refresh');
  });
