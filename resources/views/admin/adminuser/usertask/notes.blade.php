        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="profile-new-client">
             <h2>Notes</h2>
             <a href="#" class="add-task-link" id="fire-modal-2" style="display: none;">+ Add New</a>
                    ...
                    <tr>
                      ...
                      <td>
                        <?php if($v->type == 'Note' && $v->is_edit) { ?>
                          ...
                          <a href="{{url('admin/usertask/deletenote/')}}/{{$v->id}}/{{$admintask->id}}" class="action_btn" data-toggle="tooltip" title="Delete" onclick="return window.confirm('Are you sure you want to delete?');"><img src="{{ url('/') }}/assets/images/icons/case-icon3.svg"></a>
                        <?php } ?>
                      </td>
                    </tr>
                    <?php    
...
$(document).ready(function(){
  ...
  $('.saveclientinfo_form').on('click', function(e){
    ...
    var case_id = $('.modalformpart input[name="case_id"]').val();
    var note = $('.modalformpart textarea[name="note"]').val();
    var _token = $('input[name="_token"]').val();
    $.ajax({
      type:"post",
      url:"{{ url('admin/usertask/add_new_notes') }}",
      ...
      {       
        ...
        if(res.status) {
          alert(res.msg)
          window.location.href = "{{ url('admin/usertask/notes') }}/{{ $admintask->id }}";
        }
        else {
          alert(res.msg)
        }
