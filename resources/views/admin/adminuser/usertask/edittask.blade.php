...
<section class="section client-listing-details task-new-header">
  <div class="section-header">
    <h1><a href="{{ url('admin/usertask') }}"><span>Task /</span></a> Update Task</h1>
  ...
  <div class="section-body">
  		<div class="row">
  			<div class="col-md-12">
  				<div class="card">
        <form action="{{ url('admin/usertask/update_task2') }}" method="post" class="needs-validation" novalidate="">
          ...
          <div class="card-body">
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <!-- <select class="selectpicker" required="" name="vauser">
                  <option value="0">Select</option>
                  <option value="{{$current_user->id}}" data-firm_id="0" @if ($current_user->id == $admintask->firm_admin_id)
                    selected="selected"
                  @endif>{{$current_user->name}}</option>
                  <?php foreach ($firmadmins as $key => $value) { ?>
                    
                  <option value="{{$value->id}}" data-firm_id="{{$value->firm_id}}"  
                  @if ($value->id == $admintask->firm_admin_id)
                    selected="selected"
                  @endif
                  >{{$value->name}}</option>
                  <?php } ?>
                ...
                <label class="custom-switch mt-2" style="padding-left: 0px;">
                    <input type="checkbox" name="firmadminsx" value="{{$current_user->id}}" class="custom-switch-input" <?php if(empty($admintask->client_task)) { echo 'checked'; } ?>> 
                    <span class="custom-switch-indicator"></span> 
                    <span class="custom-switch-description"></span>
                  </label>
                  <input type="hidden" name="vauser" value="{{$current_user->id}}">
                <div class="invalid-feedback">Please select User Type!</div>
              </div>
            </div> 
            <div class="form-group row mb-4 clientselecter"  <?php if(empty($admintask->client_task)) { echo 'style="display: none;"'; } ?>>
                ...
                <select class="selectpicker" name="firmclient" data-live-search="true">
                  ...
                  if(!empty($firmclient)) {
                    foreach ($firmclient as $key => $value) { ?>
                      <option value="{{$value->id}}" data-firm_id="{{$value->firm_id}}" @if ($value->id == $admintask->client_task)
                    selected="selected"
          	...
          	<div class="form-group row mb-4">
          		...
          		<div class="col-sm-12 col-md-7">
                <input type="text" placeholder="Subject" name="task" class="form-control" required="" value="{{$admintask->task}}"> 
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <textarea placeholder="Description" name="description" class="form-control" required="">{{$admintask->mytask}}</textarea>
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-4 col-md-2 input-group">
                <input type="text" placeholder="Due Date" name="due_date" class="form-control datepicker1" required="" value="{{$admintask->due_date}}" style="
                    z-index: 1;
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <select class="selectpicker" required="" name="status">
                  <option value="0" <?php if($admintask->status == 0){ echo "selected"; } ?>>Open</option>
                  <option value="1" <?php if($admintask->status == 1){ echo "selected"; } ?>>Complete</option>
                </select> 
                <div class="invalid-feedback">Please select Priority Type!</div>
              </div>
            </div>
          	<div class="form-group row mb-4">
          		...
          		<div class="col-sm-12 col-md-7">
                <input type="hidden" name="id" class="form-control" value="{{$admintask->id}}">
          			@csrf
...
$(document).ready(function(){
  ...
  // $('select[name="vauser"]').on('change', function(){
  //   var v = $(this).val();
  //   var fid = $('select[name="vauser"] option[value="'+v+'"]').data('firm_id');
  //   if(fid) {
