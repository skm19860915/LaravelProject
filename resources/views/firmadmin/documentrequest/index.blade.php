...
<section class="section">
  <div class="section-header">
    ...
    <div class="section-header-breadcrumb">
      <button href="#" id="fire-modal-2" class="btn btn-primary trigger--fire-modal-2 trigger--fire-modal-1" style="display: none;">Request Document <i class="fas fa-plus"></i>
              </button>
    </div>
    ...
    <div class="row">  
      ...
      <div class="col-md-8">
        <select name="client_id" class="form-control client_Cases" required="required">
          ...
          <option value="{{$client->id}}" data-user_id="{{$client->user_id}}">{{$client->name}}</option>
          @endforeach  
          @endif
        </select>
      </div>
...
<div id="PayForTranslation" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('firm/document_request/pay_for_translation') }}/{{$id}}" method="get" id="payment-form" enctype="multipart/form-data"> 
...
var index_url = "{{url('firm/document_request/getDataDocument')}}/{{$id}}";
var srn = 0;
      ...
      columns: [
        ...
        { data: null,
          render: function(data){
            var view_button='';
            if(data.dstatus1 == 0) {
              view_button += ' <a href="{{url('firm/document_request/upload_document')}}/'+data.case_id+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="btn btn-primary viewselftuploadbtn1" title="Self upload"><i class="fas fa-file-upload"></i></i></a>'; 
            }
            else {
             view_button += '<a href="#" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="btn btn-primary viewdocbtn" title="View"><i class="fa fa-eye"></i></a>';
            }
              if(data.dstatus1 == 3) {
                if(data.quote == 0) {
                view_button += ' <a href="{{url('firm/document_request/Request_Quote')}}/'+data.did+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="btn btn-primary" title="Request a Quote"><i class="fa fa-file-invoice"></i></a>';


                view_button += ' <a href="{{url('firm/document_request/upload_document')}}/'+data.case_id+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="btn btn-primary viewselftuploadbtn1" title="Self upload"><i class="fas fa-file-upload"></i></i></a>'; 
                }
                if(data.quote == 2) {
                  view_button += ' <a href="#" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" data-quote_cost="'+data.quote_cost+'" class="btn btn-primary payfortranlation" title="Pay For Translation"><i class="fas fa-file-upload"></i></i></a>'; 
                }
              }
              return view_button;
