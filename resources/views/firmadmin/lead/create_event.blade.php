...
<section class="section">
  ...
  <div class="modalformpart" id="modal-form-part" data-reschedule="<?php echo $reschedule; ?>" style="display: none;">
    ...
    display: none;">
      ...
      <div class="col-md-8">
        <select name="event_type" class="rkselectpicker" required="required">
          <option value="">Select One</option>
          <option value="Reminder" <?php if($dateandtime['event_type'] == 'Reminder') { echo 'selected="selected"'; } ?>>Reminder</option>
          <option value="Consultation" <?php if($dateandtime['event_type'] == 'Consultation' || empty($dateandtime['event_type'])) { echo 'selected="selected"'; } ?>>Consultation</option>
    ...
    <div class="row">  
      ...
      <div class="col-md-8">
        <textarea placeholder="Event Description" name="event_description" class="form-control">{{$dateandtime['event_description']}}</textarea>
      </div>
    </div>
    <div class="row">
      <div class="col-md-4">
      ...
      <div class="col-md-8">
        <div class="input-group date form_datetime" data-date-format="mm/dd/yyyy HH:ii p">
            <input class="form-control" size="16" type="text" value="<?php echo $dateandtime['s_date']; ?>" placeholder="Event date" name="s_date" />
    ...
    <div class="row enddatewrpper">
      ...
      <div class="col-md-8">
        <div class="input-group date to_datetime" data-date-format="mm/dd/yyyy HH:ii p">
            <input class="form-control" size="16" type="text" placeholder="Event date" name="e_date" value="<?php echo $dateandtime['e_date']; ?>" />
            <span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
        </div>
      </div>
    </div>
    ...
    <div class="row">  
      ...
      <div class="col-md-8">
        ...
        <select class="rkselectpicker" name="who_consult_with" required="required" multiple>
          <option value="">select</option>
          @if (!$users->isEmpty())
          @foreach ($users as $user)
          <option value="{{$user->id}}"
            <?php if(in_array($user->id, $dateandtime['who_consult_with'])) {
              echo 'selected';
            } ?>
            >{{$user->name}}</option>
          @endforeach
...
<script>
  $(document).ready(function(){
    ...
    $('#fire-modal-2').on('click', function(){
      ...
      $('input[name="event_title"').val('{{$lead->name}} {{$lead->last_name}}');
    ...
    $('.saveclientinfo_form').on('click', function(e){
      ...
      <?php } ?>
	    $.ajax({
	    	type:"post",
	    	url:"{{ url('firm/lead/create_lead_event') }}",
          ...
          res = JSON.parse(res);
          if(res.status) {
            window.location.href = "{{ url('firm/lead/show') }}/"+lead_id;
          }
          else {
            alert(res.msg);
          }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      eventClick: function(info) {
        ...
        $('input[name="event_title"').val(info.event.title);
