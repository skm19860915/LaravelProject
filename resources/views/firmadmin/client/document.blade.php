               ...
               <?php
                if(!empty($client_doc)) {
                  foreach ($client_doc as $k => $v) { 
                   ...
                   <div class="col-md-2 col-sm-2 col-xs-6">
                    <div class="documents-pdf-box">
                     <a href="{{url('firm/client/delete_doc')}}/{{$v->id}}/{{$v->client_id}}" class="action_btn customedit_btn" onclick="return window.confirm('Are You Sure to delete this document?');" data-toggle="tooltip" title="Delete Document" style="right: 20px;top: 5px;padding-top: 1px;">
...
var index_url = "{{url('firm/document_request/getDataDocument')}}/{{$client->id}}";
var srn = 0;
      ...
      columns: [
        ...
        { data: null,
          render: function(data){
            var view_button='';
            if(data.dstatus1 == 0) {
              view_button += ' <a href="{{url('firm/document_request/upload_document')}}/'+data.case_id+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn viewselftuploadbtn1" title="Upload" data-toggle="tooltip"><i class="fas fa-upload"></i></a>'; 
            }
            else {
             view_button += ' <a href="{{url('firm/document_request/upload_document')}}/'+data.case_id+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn viewselftuploadbtn1" title="Edit" data-toggle="tooltip"><img src="{{url('assets/images/icon')}}/pencil(1)@2x.png" /></a>';
             
             var f = $("<textarea/>").html(data.document).text();
             f = JSON.parse(f);
             var url = "{{asset('storage/app')}}/"+f[0]; 
             view_button += '<a href="'+url+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn viewdocbtn4" title="View" data-toggle="tooltip" download><img src="{{url('assets/images/icon')}}/Group 557.svg" /></a>';
            }
              if(data.dstatus1 == 3) {
                if(data.quote == 0) {
                view_button += ' <a href="{{url('firm/document_request/Request_Quote')}}/'+data.did+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn" title="Request a Quote" data-toggle="tooltip"><img src="{{url('assets/images/icon')}}/ticket@2x.png" /></a>';


                view_button += ' <a href="{{url('firm/document_request/upload_document')}}/'+data.case_id+'" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" class="action_btn viewselftuploadbtn1" title="Upload" data-toggle="tooltip"><i class="fas fa-upload"></i></a>'; 
                }
                if(data.quote == 2) {
                  view_button += ' <a href="#" data-id="'+data.did+'" data-case_id="'+data.case_id+'" data-status="'+data.dstatus1+'" data-files="'+data.document+'" data-quote_cost="'+data.quote_cost+'" class="action_btn payfortranlation" title="Pay For Translation" data-toggle="tooltip"><img src="{{url('assets/images/icon')}}/ticket@2x.png" /></a>'; 
                }
              }
              return view_button;
